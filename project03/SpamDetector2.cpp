...
bool onlyDigits(const string &s)
{
    ...
    if  ( (s[FIRST_DIGIT_INDEX] == DOT) ||  
        ( s[FIRST_DIGIT_INDEX] == ZERO && s.size() != ONE_DIGIT && s[DOT_IN_SECOND_DIGIT] != DOT )
        || (s[s.size() - 1] == DOT) )
    {
        return false;
    }
...
{
    if ( (!numberOfArguments(argc - 1)) || (!existFileTest(argv[SCORE_PATH]))  ||
       (!existFileTest(argv[MESSAGE_PATH]))  || !onlyDigits(argv[THRESHOLD_PATH]))
    {
